...
int main ( void )
{
    ...
    {
        BEGIN_TEST( tm,"Rotate3", "ArrayOfChars");
        char A[]{ 'l', 'o', 'b', 'o'};
        int A_E[]{ 'b', 'o', 'l', 'o'};

        which_lib::rotate( std::begin(A), std::begin(A)+2, std::end(A) );
        EXPECT_TRUE( std::equal( std::begin(A), std::end(A), std::begin(A_E) ) );

    }

    {
        BEGIN_TEST( tm,"Rotate4", "String");
        string A{ "local"};
        string A_E{ "alloc"};

        which_lib::rotate( std::begin(A), std::begin(A)+3, std::end(A) );
        EXPECT_TRUE( std::equal( std::begin(A), std::end(A), std::begin(A_E) ) );
